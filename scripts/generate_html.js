...
function generateListHTML(json) {
  ...
  const startHTML = `<li data-required="[REQUIRED_SEX]"><span class="condensed">${name}</span><ul>`.replace(
    '[REQUIRED_SEX]',
    requiredSex
  )
    ...
    for (sex in requiredSexes) {
      for (jdx = 1; jdx < 10; jdx++) {
        ...
        if (layerDefinition !== undefined) {
          ...
          if (file !== null && file !== '') {
            dataFiles +=
              'data-layer_' + jdx + '_' + requiredSexes[sexIdx] + '="' + file + itemName.replaceAll(' ', '_') + '.png" '
          }
        } else {
          break
        }
